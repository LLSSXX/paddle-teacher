...
route_activity_application_record = Blueprint(name="route_activity_application_record", import_name=__name__)


# 学生端我的-申请记录-活动申请记录
@route_activity_application_record.route('/activity_application_record_load', methods=['POST'])
def activity_application_record_load():
    resp = {'i': {}}
    username = int(json.loads(request.values.get("username")))
    res = Activity.query.filter_by(school_number=username).all()
    information = []
    for i in res:
        i = {
...
# 学生端我的-申请记录-活动申请记录-活动申请记录详细内容
@route_activity_application_record.route('/activity_application_record_detail', methods=['POST'])
def activity_application_record_detail():
