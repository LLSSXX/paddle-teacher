...
route_classroom_check_record = Blueprint(name="route_classroom_check_record", import_name=__name__)


# 教师端我的-审核记录-教室审核记录
@route_classroom_check_record.route('/classroom_check_record_load', methods=['POST'])
def classroom_check_record_load():
    resp = {'i': {}}
    college = str(json.loads(request.values.get("college")))
    res = Classroom.query.filter(Classroom.college == college).all()
    information = []
    for i in res:
        i = {
...
# 教师端我的-审核记录-教室审核记录-查询
@route_classroom_check_record.route('/classroom_check_record_select', methods=['POST'])
def classroom_check_record_select():
    resp = {'i': {}}
    college = str(json.loads(request.values.get("college")))
    ...
    res = db.engine.execute(
        "select sequence_number,purpose,submit_time,check_method,result from classroom where college=%s and submit_time BETWEEN %s AND %s",
...
# 教师端我的-审核记录-教室审核记录-教室审核记录详细内容
@route_classroom_check_record.route('/classroom_check_record_detail', methods=['POST'])
def classroom_check_record_detail():
